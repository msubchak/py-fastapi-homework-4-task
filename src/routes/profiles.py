...
async def create_user_profile(
    ...
    stmt = (
        ...
        .options(selectinload(UserModel.profile), selectinload(UserModel.group))
    )
    ...
    current_user = (await db.execute(
        select(UserModel).where(UserModel.id == current_user_id).options(selectinload(UserModel.group))
    )).scalar_one_or_none()
